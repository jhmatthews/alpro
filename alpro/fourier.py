...
def pga_massive(s, Nsamples=200000, f_res=4,
    ...
    domain_old = s.domain.resample_box(
        new_z_use,
        interp1d_kwargs={
            "kind": "nearest",
            "fill_value": 0,
    ...
    Bxnew = np.concatenate(
        (s.domain.Bx, np.zeros(len(zcen) - len(s.domain.Bx))))
    Bynew = np.concatenate(
        (s.domain.By, np.zeros(len(zcen) - len(s.domain.By))))
    omegap = alpro.models.omega_p(s.domain.ne)
    omegap = np.concatenate(
        (s.domain.omega_p, np.zeros(len(zcen) - len(s.domain.omega_p))))

    if pol == "x" or pol == "both":
        Ex, Px, kfreq, corr = get_pga_autocorr(
            zcen, s.g_a, Bxnew, mass=s.mass, mode="massive", return_autocorr=True, transform_type=1, coord_units="kpc")

    if pol == "y" or pol == "both":
        Ey, Py, kfreq, corr = get_pga_autocorr(
            zcen, s.g_a, Bynew, mass=s.mass, mode="massive", return_autocorr=True, transform_type=1, coord_units="kpc")
...
def physical_from_numerics(s, Nsamples=100000, pad_factor=10.0, pol="both"):
    ...
    interp_omega = interp1d(phi,
                            s.domain.omega_p[start:N],
                            kind="nearest",
                            fill_value=1e-20,
                            bounds_error=False)
...
def pga_massless(s, Nsamples=200000, f_res=4, pol="both"):
    ...
    phicen, omegap, Bx, By = physical_from_numerics(
        s, Nsamples=Nsamples, pad_factor=pad_factor, pol=pol)

    if pol == "x" or pol == "both":
        E, Px, k = get_pga_autocorr(
            phicen, s.g_a, Bx, omegap=omegap, mode="massless")
    if pol == "y" or pol == "both":
        E, Py, k = get_pga_autocorr(
            phicen, s.g_a, By, omegap=omegap, mode="massless")
