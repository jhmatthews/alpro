...
def Losc(energy, B, ne, mass, g_a):
    ...
    L = 2.0 * np.pi / (delta * np.sqrt(1.0 + th**2)) / \
        unit.unit_length_natural / unit.kpc
    return (L)
...
def P_gamma_analytic(energy, B, ne, mass, g_a, L):
    ...
    # deal with the case where energy is infinite by silencing ZeroDivision
    # warning
    if np.any(np.isinf(energy)):
        with np.errstate(invalid='ignore'):
            phase = delta * L * unit.kpc * \
                unit.unit_length_natural * np.sqrt(1.0 + th**2)
            amplitude = th**2 / (1.0 + th**2)

        sin_term = np.sin(phase)
    else:
        phase = delta * np.sqrt(1.0 + th**2) * L * \
            unit.kpc * unit.unit_length_natural
        amplitude = th**2 / (1.0 + th**2)
        sin_term = np.sin(phase)
