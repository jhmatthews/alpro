...
def get_transforms(x, g, B, omegap=None, mass=None, mode="massive", transform_type = 1, coord_units="kpc"):
	'''
	...
	if coord_units == "kpc":
		...
		raise ValueError("Did not understand unit {}. Must be 'kpc' or 'natural'".format(coord_units))
	...
	if mode == "massive":
		...
		if omegap is None:
			raise ValueError("get_transforms: omegap kwarg must be set in massless case!")
...
def get_energy_samples(x, mass=None, omegap=None, mode="massive", coord_units="kpc"):
...
def get_pga_transforms(x, g, B, omegap=None, mass=None, mode="massive", transform_type=1, coord_units="kpc", return_transforms=True):
	'''
	...
	kfreq, dsx, dcx = get_transforms(xnat, g, B, omegap=omegap, mass=mass, mode=mode, transform_type=transform_type, coord_units="natural")
	P = (dsx**2) + (dcx**2)
...
def get_pga_autocorr(x, g, B, omegap=None, mass=None, mode="massive", return_autocorr=False, transform_type=1, coord_units="kpc"):
	'''
