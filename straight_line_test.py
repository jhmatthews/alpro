...
class Bfield_simple:
	def __init__(self, B0, Lmax, kind="linear"):
		self.B0 = B0
		self.Lmax = Lmax
		if kind == "linear":
			self.get_B = self._get_linear_B
		else:
			self.get_B = self._get_cosine_B

	def _get_linear_B(self, z):
		Bx = (self.B0/100) + (self.B0 * (self.Lmax - z)/self.Lmax)
		By = (self.B0/100) + (self.B0 * z/self.Lmax)
		return (Bx, By)

	def _get_cosine_B(self, z):
		Bx = self.B0 * np.cos(z/self.Lmax * 2.0 * np.pi)
		By = self.B0 * np.sin(z/self.Lmax * 2.0 * np.pi)
		return (Bx, By)
...
for kind in kinds:
	...
	Bfield_model = Bfield_simple(1e-5, Lmax, kind=kind)
		...
		for g_a in g_as:
			for mass in np.logspace(-11,-8,1):
				...
				#print (myL, myL-L, Lmax-EPSILON, Lmax, 100 - np.max(Ls))
					...
					savename = "{}_Lmax{}_{}{:03d}".format(kind, Lmax, mode, il)
